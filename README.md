...
| JWT algorithm | HS256 explicitly set on both sign and verify to prevent alg:none attacks |
...
| GDPR | Dataset contains no PII. User credentials (email, password hash) are stored but no DELETE /users endpoint exists |

---
The API has been tested against the most relevant risks from OWASP API Security Top 10:2023 via a dedicated security test collection generated by an AI agent.
